...
export default function EditCustomerModal({ isOpen, onClose, onSuccess, customer, branches }: EditCustomerModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else { setError(data.message || 'Gagal mengupdate customer') }
    } catch { setError('Terjadi kesalahan saat mengupdate customer') }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={formData.pic} onChange={(e) => setFormData({ ...formData, pic: e.target.value })} className={inputClass} />
            </div>
            <div>
            ...
            <div className="flex items-center gap-3 pt-6">
              <input type="checkbox" id="edit-customer-active" checked={formData.isActive} onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })} className="w-4 h-4 text-emerald-500 border-gray-300 rounded focus:ring-emerald-500" />
              <label htmlFor="edit-customer-active" className="text-sm font-medium text-gray-700">Aktif</label>
            </div>
          </div>
          <div>
