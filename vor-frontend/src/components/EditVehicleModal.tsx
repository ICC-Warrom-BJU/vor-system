  ...
  useEffect(() => {
    if (vehicle) {
      setFormData({
        ...
        tonase: vehicle.tonase?.toString() || '',
        kubikasi: vehicle.kubikasi?.toString() || '',
        nomorRangka: vehicle.nomorRangka || '',
        detailUnit: vehicle.detailUnit || '',
        targetRevenue: vehicle.targetRevenue?.toString() || '',
        nomorLambung: vehicle.nomorLambung || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(detail || data.message || 'Gagal mengupdate kendaraan')
      }
    } catch (err) {
      setError('Terjadi kesalahan saat mengupdate kendaraan')
    } finally {
      setLoading(false)
    }
  }
