...
// Status codes sesuai dengan master status yang ada
const statusCodes = ['UTI', 'RFU', 'BD', 'C', 'MB', 'RB', 'AM', 'BT']
...
async function main() {
  ...
  for (const date of dates) {
    ...
    for (const vehicle of vehicles) {
      const randomStatus = statusCodes[Math.floor(Math.random() * statusCodes.length)]
      const randomConfidence = [100, 75, 50, 25][Math.floor(Math.random() * 4)]
      const masterStatus = masterStatuses.find(ms => ms.code === randomStatus)
